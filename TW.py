...
banner = f"""{M}
   ...
   {M}• {P}Author {M}: {P}ALIENRAZOR
   {M}• {P}Github {M}: {K}Alienrazor
   {M}• {P}Team   {M}: {H}Alone Cyber

 {M}• {P}Paste Link Video Twitter {M}({H}https://mobile.twitter.com/balbalbal{M})"""
	...
	def dow_tw(self, id_vid, nama_vid):
		...
		try:
			...
			with open(f"{path}/{nm_vid}.mp4", "wb") as sv:
				sv.write(run)
				print (f'\n {M}• {P} Saved it {M}: {H}{path}/{nm_vid}.mp4')
		except:
